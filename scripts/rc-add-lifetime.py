...
Creates the com.jackwallner.sheepshead.lifetime product on the App Store app record,
...
Usage: python3 scripts/rc-add-lifetime.py   (key read from ~/.sheepshead_credentials)
"""
...
PRODUCT_ID = "com.jackwallner.sheepshead.lifetime"
BASE = "https://api.revenuecat.com/v2"


def secret_key() -> str:
    for line in (Path.home() / ".sheepshead_credentials").read_text().splitlines():
        if line.startswith("RC_SECRET_KEY"):
            return line.split("=", 1)[1].strip().strip('"').strip("'")
    sys.exit("RC_SECRET_KEY not found in ~/.sheepshead_credentials")
...
def main() -> None:
    ...
    project = projects[0] if len(projects) == 1 else next(
        p for p in projects if "sheepshead" in p["name"].lower()
    )
    pid = project["id"]
    ...
    if product is None:
        product = request("POST", f"/projects/{pid}/products", {
            ...
            "app_id": app["id"],
            "type": "one_time",
            "display_name": "Sheepshead+ Lifetime",
        })
