...
struct FlashcardDrillView: View {
    ...
    @AppStorage("sheepshead.hasSwipedDeck") private var hasSwipedDeck = false
...
struct FlipCardFace: View {
    ...
    private var front: some View {
        SheepsheadCardFace(accent: accent, eyebrow: "SHEEPSHEAD") {
            VStack(spacing: 18) {
                Spacer(minLength: 0)
    ...
    private var back: some View {
        SheepsheadCardFace(accent: accent, eyebrow: "THE CALL") {
            VStack(spacing: 14) {
                if let verdict {
...
/// The sheepshead-card chrome both faces share: ivory surface, double frame like
/// the printed card, corner pips, and a faint suit watermark so the
/// open space feels designed instead of empty.
private struct SheepsheadCardFace<Content: View>: View {
    let accent: Color
    let eyebrow: String
