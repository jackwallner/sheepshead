...
struct SheepsheadMinuteView: View {
    @StateObject private var store = SheepsheadMinuteStore.shared

    private var today: Date { Date() }
    private var challenge: SheepsheadMinuteChallenge { SheepsheadMinuteContent.challenge(for: today) }
    private var todayResult: SheepsheadMinuteResult? { store.result(for: today) }
    ...
    var body: some View {
        ...
        .navigationTitle("Sheepshead Minute")
    ...
    private var hero: some View {
        VStack(spacing: 14) {
            ...
            VStack(spacing: 5) {
                Text("Today's Sheepshead Minute")
                    .font(Theme.display(27))
                    .foregroundStyle(Theme.ink)
                Text("The same five original questions for every member: two hand reads, one bury decision, and two trick calls.")
                    .font(.subheadline)
            ...
            if let todayResult {
                NavigationLink {
                    SheepsheadMinuteResultView(result: todayResult)
                } label: {
                    Text("View Today's \(todayResult.score)/\(todayResult.total)").primaryCTA()
                }
            } else {
                NavigationLink {
                    QuickSessionView(sheepsheadMinute: challenge)
                } label: {
    ...
    private func archiveRow(_ date: Date) -> some View {
        ...
        NavigationLink {
            if let result {
                SheepsheadMinuteResultView(result: result)
            } else {
                QuickSessionView(sheepsheadMinute: SheepsheadMinuteContent.challenge(for: date))
            }
        } label: {
            HStack(spacing: 12) {
...
struct SheepsheadMinuteResultView: View {
    ...
    private var scoreCard: some View {
        VStack(spacing: 8) {
            ...
            Text(result.score == result.total ? "Perfect minute!" : "Minute complete")
                ...
                .foregroundStyle(Theme.ink)
            Text("Sheepshead Minute \(result.shortDate)")
                .font(.subheadline)
                .foregroundStyle(Theme.inkSecondary)
        }
        .frame(maxWidth: .infinity)
    ...
    private var breakdown: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("BY SKILL")
                ...
                .kerning(1.4)
                .foregroundStyle(Theme.inkSecondary)
            ForEach(SheepsheadMinuteCategory.allCases) { category in
                HStack(spacing: 12) {
    ...
    private func color(for category: SheepsheadMinuteCategory) -> Color {
    ...
    private func recordCompletionIfNeeded() {
        ...
        progress.recordSession(drillID: SheepsheadMinuteContent.drill.id)
